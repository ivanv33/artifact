...
def _make_run(tmp_path: Path, fixture: str, outputs: list[str], **kwargs) -> Path:
    src = FIXTURES / fixture
    dst = tmp_path / fixture
    if not dst.exists():
        shutil.copytree(src, dst, ignore=shutil.ignore_patterns("runs", "outs"))
    return run(dst, executor=_FakeExecutor(outputs), **kwargs)


def test_promote_copies_full_run_directory(tmp_path, sequential_run_ids):
    run_dir = _make_run(tmp_path, "trivial", ["hello.md"], params={}, inputs={})
...
def test_promote_is_a_copy_not_a_symlink(tmp_path, sequential_run_ids):
    run_dir = _make_run(tmp_path, "trivial", ["hello.md"], params={}, inputs={})
    artifact_dir = run_dir.parent.parent
...
def test_promote_updates_manifest_in_both_locations(tmp_path, sequential_run_ids):
    run_dir = _make_run(tmp_path, "trivial", ["hello.md"], params={}, inputs={})
    artifact_dir = run_dir.parent.parent
...
def test_promote_overwrites_existing_label(tmp_path, sequential_run_ids):
    r1 = _make_run(tmp_path, "trivial", ["hello.md"], params={}, inputs={})
    artifact_dir = r1.parent.parent
    promote(artifact_dir, r1.name, label="latest")

    r2 = _make_run(tmp_path, "trivial", ["hello.md"], params={}, inputs={})
